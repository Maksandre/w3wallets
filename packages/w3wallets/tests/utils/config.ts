...
const getConfig = () => {
  const { ETHEREUM_PRIVATE_KEY, ETHEREUM_MNEMONIC, SUBSTRATE_SEED, ECLIPSE_PRIVATE_KEY } =
    process.env;

  if (!ETHEREUM_PRIVATE_KEY || !SUBSTRATE_SEED || !ECLIPSE_PRIVATE_KEY || !ETHEREUM_MNEMONIC)
