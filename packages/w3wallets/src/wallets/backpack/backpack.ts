...
export class Backpack extends Wallet {
  ...
  async onboard(network: Network, privateKey: string) {
    this.currentAccountId++;
    this.maxAccountId++;
    return this._importAccount(network, privateKey, true);
  }

  async addAccount(network: Network, privateKey: string) {
    this.maxAccountId++;
  ...
  async setRPC(network: Network, rpc: string) {
  ...
  private async _importAccount(
    ...
    await this.page
      .getByTestId("__CAROUSEL_ITEM_0__")
